...
class FileTextExtractorTest(TestCase):
    ...
    def test_is_supported_file(self):
        ...
        unsupported_files = ["document.exe", "document.bin", "document.mp3", "document.jpg", "document.mp4"]

        for filename in supported_files:
            self.assertTrue(self.extractor.is_supported_file(filename), f"Файл {filename} должен поддерживаться")

        for filename in unsupported_files:
            self.assertFalse(self.extractor.is_supported_file(filename), f"Файл {filename} не должен поддерживаться")
...
class DocumentFileServiceTest(TestCase):
    ...
    def setUp(self):
        ...
        self.category = DocumentCategory.objects.create(name="Тестовая категория", description="Описание категории")
    ...
    def test_create_document_from_text_file(self):
        ...
        document, error = self.file_service.create_document_from_file(
            uploaded_file=uploaded_file, title="Тестовый документ", category=self.category, author=self.user
        )
    ...
    def test_create_document_without_title(self):
        ...
        document, error = self.file_service.create_document_from_file(uploaded_file=uploaded_file, author=self.user)
    ...
    def test_create_document_from_large_file(self):
        ...
        uploaded_file = SimpleUploadedFile("large_file.txt", large_content.encode("utf-8"), content_type="text/plain")
    ...
    def test_update_document_from_file(self):
        ...
        original_document = Document.objects.create(
            title="Исходный документ", content="Исходное содержимое", author=self.user
        )
        ...
        success, error = self.file_service.update_document_from_file(
            document=original_document, uploaded_file=uploaded_file
        )
...
Обычный текст с python программированием."""
        ...
        document, error = self.file_service.create_document_from_file(
            uploaded_file=uploaded_file, title="Markdown документ", author=self.user
        )
...
<body>
    ...
    <div>
        ...
        document, error = self.file_service.create_document_from_file(
            uploaded_file=uploaded_file, title="HTML документ", author=self.user
        )
...
HTML,HTML,Разметка,Язык разметки веб-страниц"""
        ...
        document, error = self.file_service.create_document_from_file(
            uploaded_file=uploaded_file, title="Языки программирования", author=self.user
        )
    ...
    def test_document_creation_saves_file_path(self):
        ...
        document, error = self.file_service.create_document_from_file(
            uploaded_file=uploaded_file, title="Документ с файлом", author=self.user
        )
    ...
    def test_document_update_preserves_metadata(self):
        ...
        # Создаем исходный документ
        original_document = Document.objects.create(
            title="Документ для обновления", content="Исходное содержимое", author=self.user, category=self.category
        )
